...
class MTEF:
    ...
    def readRecord(self):
        ...
        while True:
            ...
            if record >= RecordType.FUTURE:
                fut_len = self.reader.read(1)
                if not fut_len:
                    break
                skipFutureLength = Helper.bytes2int(fut_len) # uint8
    ...
    def makeLatex(self, ast):
        ...
        buf = ''
        _c = lambda i: ast.children[i] if hasattr(ast, 'children') and i < len(ast.children) else None
        ...
        if ast.tag == RecordType.ROOT:
            ...
            if tmpl.selector == SelectorType.tmANGLE:
                ...
                leftSlot, _ = self.makeLatex(leftAST)
                rightSlot, _ = self.makeLatex(rightAST)
                leftStr = f"\\left{leftSlot}" if leftSlot else "\\left."
                rightStr = f"\\right{rightSlot}" if rightSlot else "\\right."
                buf += f"{leftStr} {mainSlot} {rightStr}"
                return buf, None
            ...
            if embellType == EmbellType.emb1DOT:
                ...
                embellStr = " \\bar "
            elif embellType == EmbellType.embRARROW:
                embellStr = " \\vec "
            elif embellType == EmbellType.embTILDE:
                embellStr = " \\tilde "
            else:
                pass
